...
func Conn(host string, port int) (*SSDB, error) {
	remote := host + ":" + strconv.Itoa(port)
	con, err := net.Dial("tcp", remote)
	return &SSDB{con, false, [][]byte{}, []byte{}}, err
}
	...
	case "zrange":
		...
		if len(resp) > 0 && resp[0] == "ok" {
			...
			for i := 1; i < size-1; i += 2 {
				val, _ := strconv.Atoi(resp[i+1])
				data[resp[i]] = int64(val)
			}
			return data, nil
		}
	default:
...
func (ssdb *SSDB) Zrange(key string, offset, limit int) (map[string]int64, error) {
	ret, err := ssdb.Request("zrange", key, strconv.Itoa(offset), strconv.Itoa(limit))
	if ret != nil {
		return ret.(map[string]int64), err
	}
